...
exports.handleMomoIpn = async (orderIdInput, transactionCodeInput, resultCodeInput, transIdInput, providerRefInput) => {
  const transactionCode = typeof orderIdInput === 'string' ? orderIdInput.trim() : typeof transactionCodeInput === 'string' ? transactionCodeInput.trim() : '';
  const resultCode = resultCodeInput != null ? String(resultCodeInput) : '';
  const providerRef = typeof transIdInput === 'string' ? transIdInput.trim() : typeof providerRefInput === 'string' ? providerRefInput.trim() : null;
