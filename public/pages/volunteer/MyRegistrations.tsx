...
const MyRegistrations: React.FC = () => {
  ...
  const handleCancelRegistration = (id: number) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire(
          'Đã hủy!',
          'Hồ sơ đăng ký của bạn đã được xóa.',
          'success'
        );
      }
    });
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'Confirmed':
        return <span className="badge bg-success-subtle text-success border border-success-subtle rounded-pill py-2 px-3">
          <CheckCircle2 size={16} className="me-1" /> Đã chấp nhận
        </span>;
      case 'Pending':
        return <span className="badge bg-warning-subtle text-warning border border-warning-subtle rounded-pill py-2 px-3">
          <Clock size={16} className="me-1" /> Chờ phê duyệt
        </span>;
      case 'Rejected':
        return <span className="badge bg-danger-subtle text-danger border border-danger-subtle rounded-pill py-2 px-3">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        ...
        <div className="d-flex gap-2">
          ...
          <div className="dropdown">
            <button className="btn btn-sm btn-outline-success rounded-pill px-3 shadow-sm dropdown-toggle" type="button" data-bs-toggle="dropdown">
            ...
            <ul className="dropdown-menu shadow border-0 rounded-4 mt-2 px-2 py-2">
              <li><button className={`dropdown-item rounded-pill ${filter === 'All' ? 'active bg-success' : ''}`} onClick={() => setFilter('All')}>Tất cả</button></li>
              <li><button className={`dropdown-item rounded-pill ${filter === 'Confirmed' ? 'active bg-success' : ''}`} onClick={() => setFilter('Confirmed')}>Đã chấp nhận</button></li>
              <li><button className={`dropdown-item rounded-pill ${filter === 'Pending' ? 'active bg-success' : ''}`} onClick={() => setFilter('Pending')}>Chờ phê duyệt</button></li>
              <li><button className={`dropdown-item rounded-pill ${filter === 'Rejected' ? 'active bg-success' : ''}`} onClick={() => setFilter('Rejected')}>Đã từ chối</button></li>
            </ul>
          </div>
        </div>
      </div>
              ...
              {filteredRegistrations.length === 0 ? (
                ...
                filteredRegistrations.map((reg) => (
                  <tr key={reg.id} className="transition-all">
                    <td className="py-4 px-4">
                      <div className="d-flex align-items-center">
                        <div className="me-3 position-relative" style={{ width: '64px', height: '64px' }}>
                          <img 
                            src={reg.thumbnail || '/images/placeholder.jpg'}
                    ...
                    <td className="py-4 px-4 text-center text-muted fw-medium">
